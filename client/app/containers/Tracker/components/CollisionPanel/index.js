...
function CollisionPanel({ data, pointData, collisionTool, toggleForce, showForce }) {
  const Panel = panelMapping[collisionTool.mode];

  return (
    <CollisionPanelWrapper>
      <Panel data={data} pointData={pointData} toggleForce={toggleForce} showForce={showForce} />
    </CollisionPanelWrapper>
  );
}
